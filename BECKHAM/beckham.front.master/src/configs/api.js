...
export const api_url = is_production
  ? 'https://api.botfood.id/api/v1/solana/beckham'
  : 'http://localhost/beckham.api'
...
function getHeaderData(attribute = null) {
  ...
  const headers = {
    ...(api_public_secret && { 'X-Token-Public': api_public_secret }),
...
export function getBlob(url, header = '') {
  return axios
    .get(`${api_url}/${url}`, { responseType: 'blob' })
    .then((response) => {
      return response;
    })
    .catch((error) => {
